    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        if (data[0]?.product_id) {
          newView = data[0]?.category === "Accessories" ? "accessories" : "phones";
        } else if (data[0]?.plan_id) {
          newView = data[0]?.speed_down ? "internet" : "plans";
        }
      } else if (data.plans && Array.isArray(data.plans)) {
        // Internet response with qualification status
        if (data.plans[0]?.speed_down || data.plan_type) {
          newView = "internet";
        } else if (data.plans[0]?.plan_id) {
          newView = "plans";
        }
        ...
        cartState = data;
        updateCartBadge();
      } else if (data.total_products !== undefined) {
        newView = "inventory";
      } else if (data.stores !== undefined && Array.isArray(data.stores)) {
        newView = "stores";
      ...
      appContainer.innerHTML = `<pre style="padding: 20px; white-space: pre-wrap;">${text}</pre>`;
    }
  }
};
